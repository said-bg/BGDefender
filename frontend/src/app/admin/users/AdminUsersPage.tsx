...
import styles from '@/features/admin/users/AdminUsersPage.module.css';
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.filters}>
        ...
        >
          <option value="all">{t('users.filterAllPlans', { defaultValue: 'All plans' })}</option>
          <option value={UserPlan.FREE}>{t('levels.free', { defaultValue: 'Free' })}</option>
        ...
        >
          <option value="all">{t('users.filterAllRoles', { defaultValue: 'All roles' })}</option>
      ...
      {loading ? (
        <p className={styles.statusMessage}>{t('loading', { defaultValue: 'Loading admin data...' })}</p>
        ...
        <section className={styles.list}>
          {preparedUsers.map((user) => {
            ...
            const planBadgeClass =
              user.plan === UserPlan.PREMIUM ? styles.badgePremium : styles.badgeFree;
              ...
              <article key={user.id} className={styles.card}>
                ...
                <div className={styles.actions}>
                  {showPlan ? (
                    ...
                    >
                      {user.plan === UserPlan.PREMIUM
                        ? t('users.removePremium', { defaultValue: 'Remove premium' })
                        : t('users.grantPremium', { defaultValue: 'Grant premium' })}
                    </button>
                  ) : null}
                  ...
                  {user.role !== UserRole.ADMIN ? (
                    ...
                    >
                      {user.role === UserRole.CREATOR
                        ? t('users.removeCreator', { defaultValue: 'Remove creator' })
                        : t('users.grantCreator', { defaultValue: 'Grant creator' })}
                    </button>
                  ) : null}

                  <button
                    type="button"
                    className={`${styles.actionButton} ${!user.isActive ? styles.dangerAction : ''}`}
